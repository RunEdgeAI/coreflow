...
static const vx_char name[VX_MAX_TARGET_NAME] = "coreflow.tflite.rt";
...
extern "C" VX_API_ENTRY vx_status VX_API_CALL vxPublishKernels(vx_context context)
{
    ...
    vx_uint32 p = 0, k = 0;
    for (k = 0; k < num_target_kernels; k++)
    {
        vx_kernel kernel = 0;

        kernel = vxAddUserKernel(context, target_kernels[k]->name, target_kernels[k]->enumeration,
                                 target_kernels[k]->function, target_kernels[k]->numParams,
                                 target_kernels[k]->validate, target_kernels[k]->initialize,
                                 target_kernels[k]->deinitialize);
        if (VX_SUCCESS == vxGetStatus((vx_reference)kernel))
        {
            status = VX_SUCCESS;  // temporary
            for (p = 0; p < target_kernels[k]->numParams; p++)
            {
                status =
                    vxAddParameterToKernel(kernel, p, target_kernels[k]->parameters[p].direction,
                                           target_kernels[k]->parameters[p].data_type,
                                           target_kernels[k]->parameters[p].state);
                if (status != VX_SUCCESS)
                {
                    vxAddLogEntry((vx_reference)context, status,
                                  "Failed to add parameter %d to kernel %s! (%d)\n", p,
                                  target_kernels[k]->name, status);
                    break;
                }
            }
            if (status == VX_SUCCESS)
...
extern "C" VX_API_ENTRY vx_status VX_API_CALL vxUnpublishKernels(vx_context context)
{
    ...
    for (k = 0; k < num_target_kernels; k++)
    {
        vx_kernel kernel = vxGetKernelByName(context, target_kernels[k]->name);
