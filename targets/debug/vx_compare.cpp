            ...
            if (VX_SUCCESS == status && vxFindOverlapRectangle(&rect_a, &rect_b, &rect) == vx_true_e)
            {
                ...
                for (p = 0; p < a_planes; p++)
                {
                    for (y = 0; y < a_addrs[p].dim_y; y += a_addrs[p].step_y)
                    {
                        ...
                        {
                            uint8_t* a_ptr = (uint8_t*)vxFormatImagePatchAddress2d(a_base_ptrs[p], x, y, &a_addrs[p]);
                            uint8_t* b_ptr = (uint8_t*)vxFormatImagePatchAddress2d(b_base_ptrs[p], x, y, &b_addrs[p]);
