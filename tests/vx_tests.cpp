    ...
    if (vxGetStatus((vx_reference)context) == VX_SUCCESS)
    {
        ...
        if (vxGetStatus((vx_reference)graph) == VX_SUCCESS)
        {
            ...
            for (y = 0; y < height; y++)
            {
                for (x = 0; x < width; x++)
                {
                    vx_uint8 *img0 = reinterpret_cast<vx_uint8*>(vxFormatImagePatchAddress2d(base0, x, y, &image0_addr));
                    vx_uint8 *img1 = reinterpret_cast<vx_uint8*>(vxFormatImagePatchAddress2d(base1, x, y, &image1_addr));
            ...
            for (i = 0; i < dimof(subtests); i++)
            {
                ...
                for (y = 0; y < height; y++)
                {
                    for (x = 0; x < width; x++)
                    {
                        vx_uint8 *img = reinterpret_cast<vx_uint8*>(vxFormatImagePatchAddress2d(base, x, y, &image_addr));
                                              ...
                                              vx_uint32 period)
{
    ...
    if (format == VX_DF_IMAGE_U8)
    {
        ...
        for (y = 0; y < height; y++)
        {
            for (x = 0; x < width; x++)
            {
                vx_uint8 *img = reinterpret_cast<vx_uint8*>(vxFormatImagePatchAddress2d(base, x, y, &addr));
    ...
    else if (format == VX_DF_IMAGE_S16)
    {
        ...
        for (y = 0; y < height; y++)
        {
            for (x = 0; x < width; x++)
            {
                vx_int16 *img = reinterpret_cast<vx_int16*>(vxFormatImagePatchAddress2d(base, x, y, &addr));
                       ...
                       for (iscale = 0; iscale < op_tests[iops].n_scales; iscale++)
                       {
                           ...
                           for (y = 0; y < height; y++)
                           {
                             for (x = 0; x < width; x++)
                             {
                               ...
                               vx_int32 ref = op_tests[iops].refop(v0, v1, scale, dest_format, (vx_convert_policy_e)policy);
