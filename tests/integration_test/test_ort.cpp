/**
 * @file test_ort.cpp
 * @brief Test Onnx Runtime Target
 * @version 0.1
 * @date 2025-01-29
 *
...
protected:
    vx_context context;
    vx_graph graph;
    std::string model_path = "./tests/raw/model.onnx";
    ...
    {
        // Write binary ONNX model file with MatMul node
        // Note: In real implementation, use ONNX protobuf to create model
        std::ofstream ofs(model_path, std::ios::binary);
        // Write ONNX model content
        ofs.close();
    }
};
...
TEST_F(OrtIntegrationTest, OnnxMatMul)
{
    ...
    vx_kernel kernel = vxGetKernelByEnum(context, VX_KERNEL_ORT_CPU_INF);
