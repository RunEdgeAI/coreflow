...
//     if (arr->capacity > 0)
//     {
//         res = ownAllocateMemory(context, &arr->memory);
//     }
//     return res;
// }
...
//             if (ownAddAccessor(context, size, usage, *ptr, &arr->base, &a, stride_save) == vx_true_e)
//             {
//                 vx_size offset;
...
//         if (ownAddAccessor(context, size, usage, *ptr, &arr, &a, stride_save) == vx_true_e)
//         {
//             *ptr = context->accessors[a].ptr;
...
//         vx_bool internal = ownFindAccessor(context, ptr, &index);
...
//     vx_size size = (end - start) * arr->item_size;
//     if (ownMemoryMap(context, (vx_reference)arr, size, usage, mem_type, flags, &extra, (void **)&buf, map_id) == vx_true_e)
//     {
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseArray(vx_array *a)
{
    ...
    // return ownReleaseReferenceInt((vx_reference_t **)a, VX_TYPE_ARRAY, VX_EXTERNAL, nullptr);
...
VX_API_ENTRY vx_status VX_API_CALL vxAddArrayItems(vx_array arr, vx_size count, const void *ptr, vx_size stride)
{
    ...
    if (vxIsValidArray(arr) == vx_true_e)
    {
        ...
        if(vx_true_e)// ownAllocateArray(arr) == vx_true_e)
        {
            status = VX_ERROR_INVALID_PARAMETERS;
