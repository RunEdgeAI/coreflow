                                                         ...
                                                         vx_enum user_mem_type)
{
    ...
    if (threshold->is_virtual == vx_true_e)
    {
        if (threshold->is_accessible == vx_false_e)
        {
    ...
    if (user_mem_type == VX_MEMORY_TYPE_OPENCL_BUFFER)
    {
        ...
        if (true_value_ptr)
        {
            ...
            true_value_ptr = (vx_pixel_value_t *)clEnqueueMapBuffer(threshold->context->opencl_command_queue,
        ...
        if (false_value_ptr)
        {
            ...
            false_value_ptr = (vx_pixel_value_t *)clEnqueueMapBuffer(threshold->context->opencl_command_queue,
                opencl_buf, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, size,
                0, nullptr, nullptr, &cerr);
            VX_PRINT(VX_ZONE_CONTEXT, "OPENCL: vxCopyThresholdOutput: clEnqueueMapBuffer(%p,%d) => %p (%d)\n",
    ...
    if (user_mem_type_given == VX_MEMORY_TYPE_OPENCL_BUFFER)
    {
        ...
        {
            clEnqueueUnmapMemObject(threshold->context->opencl_command_queue,
                (cl_mem)true_value_ptr_given, true_value_ptr, 0, nullptr, nullptr);
            clFinish(threshold->context->opencl_command_queue);
        }
        if (false_value_ptr_given)
        {
            clEnqueueUnmapMemObject(threshold->context->opencl_command_queue,
                (cl_mem)false_value_ptr_given, false_value_ptr, 0, nullptr, nullptr);
            clFinish(threshold->context->opencl_command_queue);
        }
    }
#endif
                                                        ...
                                                        vx_enum user_mem_type)
{
    ...
    if (user_mem_type == VX_MEMORY_TYPE_OPENCL_BUFFER)
    {
        ...
        if (lower_value_ptr)
        {
            ...
            lower_value_ptr = (vx_pixel_value_t *)clEnqueueMapBuffer(threshold->context->opencl_command_queue,
                opencl_buf, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, size,
                0, nullptr, nullptr, &cerr);
        ...
        if (upper_value_ptr)
        {
            ...
            upper_value_ptr = (vx_pixel_value_t *)clEnqueueMapBuffer(threshold->context->opencl_command_queue,
                opencl_buf, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, size,
                0, nullptr, nullptr, &cerr);
            VX_PRINT(VX_ZONE_CONTEXT, "OPENCL: vxCopyThresholdRange: clEnqueueMapBuffer(%p,%d) => %p (%d)\n",
    ...
    if (user_mem_type_given == VX_MEMORY_TYPE_OPENCL_BUFFER)
    {
        ...
        {
            clEnqueueUnmapMemObject(threshold->context->opencl_command_queue,
                (cl_mem)lower_value_ptr_given, lower_value_ptr, 0, nullptr, nullptr);
            clFinish(threshold->context->opencl_command_queue);
        }
        if (upper_value_ptr_given)
        {
            clEnqueueUnmapMemObject(threshold->context->opencl_command_queue,
                (cl_mem)upper_value_ptr_given, upper_value_ptr, 0, nullptr, nullptr);
            clFinish(threshold->context->opencl_command_queue);
        }
    }
#endif
                                                        ...
                                                        )
{
    ...
    if (user_mem_type == VX_MEMORY_TYPE_OPENCL_BUFFER)
    {
        ...
        value_ptr = (vx_pixel_value_t *)clEnqueueMapBuffer(threshold->context->opencl_command_queue,
            opencl_buf, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, size,
            0, nullptr, nullptr, &cerr);
    ...
    {
        clEnqueueUnmapMemObject(threshold->context->opencl_command_queue,
            (cl_mem)value_ptr_given, value_ptr, 0, nullptr, nullptr);
        clFinish(threshold->context->opencl_command_queue);
    }
#endif
