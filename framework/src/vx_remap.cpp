...
vx_uint32 Remap::srcWidth() const
{
    return src_width;
}

vx_uint32 Remap::srcHeight() const
{
    return src_height;
}

vx_uint32 Remap::dstWidth() const
{
    return dst_width;
}

vx_uint32 Remap::dstHeight() const
{
    return dst_height;
}

vx_status Remap::setRemapPoint(vx_uint32 dst_x, vx_uint32 dst_y, vx_float32 src_x, vx_float32 src_y)
{
    vx_status status = VX_SUCCESS;

    if (Memory::allocateMemory(context, &memory) == vx_false_e)
    {
        VX_PRINT(VX_ZONE_ERROR, "Failed to allocate memory for remap object!\n");
...
vx_status Remap::getRemapPoint(vx_uint32 dst_x, vx_uint32 dst_y, vx_float32 *src_x,
                               vx_float32 *src_y)
{
    vx_status status = VX_SUCCESS;
...
vx_status Remap::copyPatch(const vx_rectangle_t *rect, vx_size user_stride_y, void *user_ptr,
                           vx_enum user_coordinate_type, vx_enum usage, vx_enum user_mem_type)
{
    ...
    /* more bad parameters */
    if (VX_SUCCESS == status && user_mem_type != VX_MEMORY_TYPE_HOST &&
    ...
    /* more bad parameters */
    if (VX_SUCCESS == status && zero_area == vx_false_e &&
        ((0 >= memory.nptrs) || (start_x >= end_x) || (start_y >= end_y)))
    {
        status = VX_ERROR_INVALID_PARAMETERS;
    }
    ...
    if (VX_SUCCESS == status)
    {
        ...
        if (usage == VX_READ_ONLY)
        {
            ...
            for (i = start_y; i < end_y; i++)
            {
                for (j = start_x; j < end_x; j++)
                {
                    ...
                    status = getCoordValue(j, i, &coord_ptr->x, &coord_ptr->y);
                }
            }
        }
        else
        {
            ...
            for (i = start_y; i < end_y; i++)
            {
                ...
                {
                    vx_coordinates2df_t *coord_ptr = &(ptr[i * stride + j]);
                    status = setCoordValue(j, i, coord_ptr->x, coord_ptr->y);
                }
            }
        }
...
vx_status Remap::mapPatch(const vx_rectangle_t *rect, vx_map_id *map_id, vx_size *stride_y,
                          void **ptr, vx_enum coordinate_type, vx_enum usage, vx_enum mem_type)
{
    ...
    /* bad parameters */
    if ((rect == nullptr) || (map_id == nullptr) || (ptr == nullptr))
    {
        status = VX_ERROR_INVALID_PARAMETERS;
    }

    /* more bad parameters */
    if (VX_SUCCESS == status && coordinate_type != VX_TYPE_COORDINATES2DF)
    {
        status = VX_ERROR_INVALID_PARAMETERS;
    }
    ...
    if (VX_SUCCESS == status)
    {
        ...
        if (context->memoryMap((vx_reference)this, size, usage, mem_type, flags, &extra,
                               (void **)&buf, map_id) == vx_true_e)
        {
            ...
            (mem_type_requested == VX_MEMORY_TYPE_OPENCL_BUFFER) && (size > 0) && ptr && *ptr)
        {
            ...
            cl_int cerr = 0;
            cl_mem opencl_buf =
                clCreateBuffer(context->opencl_context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                               size, *ptr, &cerr);
...
vx_status Remap::unmapPatch(vx_uint32 map_id)
{
    vx_status status = VX_SUCCESS;
...
VX_API_ENTRY vx_remap VX_API_CALL vxCreateRemap(vx_context context, vx_uint32 src_width,
...
VX_API_ENTRY vx_remap VX_API_CALL vxCreateVirtualRemap(vx_graph graph, vx_uint32 src_width,
                                                       vx_uint32 src_height, vx_uint32 dst_width,
                                                       vx_uint32 dst_height)
{
    vx_remap remap = nullptr;
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryRemap(vx_remap remap, vx_enum attribute, void *ptr,
                                                vx_size size)
{
    vx_status status = VX_SUCCESS;
                                                   ...
                                                   vx_float32 src_x, vx_float32 src_y)
{
    ...
    if (Reference::isValidReference(remap, VX_TYPE_REMAP) == vx_true_e)
    {
        status = remap->setRemapPoint(dst_x, dst_y, src_x, src_y);
    }
    else
    {
        VX_PRINT(VX_ZONE_ERROR, "Not a valid object!\n");
                                                   ...
                                                   vx_float32 *src_x, vx_float32 *src_y)
{
    ...
    if (Reference::isValidReference(remap, VX_TYPE_REMAP) == vx_true_e)
    {
        status = remap->getRemapPoint(dst_x, dst_y, src_x, src_y);
    }
    else
    {
        VX_PRINT(VX_ZONE_ERROR, "Not a valid object!\n");
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyRemapPatch(vx_remap remap, const vx_rectangle_t *rect,
                                                    vx_size user_stride_y, void *user_ptr,
                                                    vx_enum user_coordinate_type, vx_enum usage,
                                                    vx_enum user_mem_type)
{
    ...
    {
        status = remap->copyPatch(rect, user_stride_y, user_ptr, user_coordinate_type, usage,
                                  user_mem_type);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxMapRemapPatch(vx_remap remap, const vx_rectangle_t *rect,
                                                   vx_map_id *map_id, vx_size *stride_y, void **ptr,
                                                   vx_enum coordinate_type, vx_enum usage,
                                                   vx_enum mem_type)
{
    vx_status status = VX_ERROR_INVALID_REFERENCE;
    ...
    if (Remap::isValidRemap(remap) == vx_true_e)
    {
        status = remap->mapPatch(rect, map_id, stride_y, ptr, coordinate_type, usage, mem_type);
    }
    return status;
}
...
VX_API_ENTRY vx_status VX_API_CALL vxUnmapRemapPatch(vx_remap remap, vx_map_id map_id)
{
    ...
    if (Remap::isValidRemap(remap) == vx_true_e)
    {
        status = remap->unmapPatch(map_id);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseRemap(vx_remap *r)
{
    ...
    if (nullptr != r)
    {
        ...
        {
            status =
                Reference::releaseReference((vx_reference *)r, VX_TYPE_REMAP, VX_EXTERNAL, nullptr);
        }
    }
