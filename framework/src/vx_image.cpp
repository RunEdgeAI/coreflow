...
VX_API_ENTRY vx_image VX_API_CALL vxCreateImageFromROI(vx_image image, const vx_rectangle_t* rect)
{
    ...
    if (ownIsValidImage(image) == vx_true_e)
    {
        ...
        else
        {
            ...
            {
                // subimage = (vx_image)ownCreateReference(image->context, VX_TYPE_IMAGE, VX_EXTERNAL, &image->context->base);
...
void ownDestructImage(vx_reference ref)
{
    ...
    {
        // ownReleaseReferenceInt((vx_reference *)&image->parent, VX_TYPE_IMAGE, VX_INTERNAL, nullptr);
    }
                                    ...
                                    vx_enum usage)
{
    ...
    else
    {
        ...
        // if (ownAddAccessor(image->context, size, usage, *ptr, &image, &a, addr_save) == vx_false_e)
        {
            status = VX_ERROR_NO_MEMORY;
    ...
    vx_uint32 flags)
{
    ...
    /* MAP mode */
    {
        ...
        if (1) //VX_MEMORY_TYPE_NONE != image->memory_type && vx_true_e == ownMemoryMap(image->context, (vx_reference)image, 0, usage, mem_type, flags, &extra, (void**)&buf, map_id))
        {
            /* use the addressing of the internal format */
