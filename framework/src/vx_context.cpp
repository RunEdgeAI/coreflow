...
vx_status Context::loadTarget(const vx_char* targetName)
{
    ...
    VX_PRINT(VX_ZONE_TARGET, "Entered %s\n", __func__);
    for (index = 0; index < VX_INT_MAX_NUM_TARGETS; index++)
    {
        if ((targets[index] == nullptr) ||
            (targets[index] && targets[index]->module.handle == VX_MODULE_INIT))
        {
            break;
        }
    }
...
#if defined(WIN32)
        _snprintf(module, VX_INT_MAX_PATH, VX_MODULE_NAME("%s"), (targetName?targetName:"openvx-target"));
        ...
        if (targets[index]->module.handle)
        {
            targets[index]->incrementReference(VX_INTERNAL);
            {
                ...
                if (targets[index]->funcs.init &&
                    ...
                    targets[index]->funcs.supports &&
                    targets[index]->funcs.process &&
                    targets[index]->funcs.verify &&
                    targets[index]->funcs.addkernel)
                {
...
vx_status Context::unloadTarget(vx_uint32 index, vx_bool unload_module)
{
    ...
    vx_target target = targets[index];
...
{
    return vxCreateContextFromPlatform(nullptr);
}
...
VX_API_ENTRY vx_context VX_API_CALL vxGetContext(vx_reference reference)
{
    ...
    if (Reference::isValidReference(reference) == vx_true_e)
    {
        context = reference->context;
    }
    else if (Context::isValidContext((vx_context)reference) == vx_true_e)
    {
...
VX_API_ENTRY vx_status VX_API_CALL vxHint(vx_reference reference, vx_enum hint, const void* data, vx_size data_size)
{
    ...
    if (Context::isValidContext((vx_context)reference) == vx_false_e && Reference::isValidReference(reference) == vx_false_e)
    {
...
VX_API_ENTRY vx_status VX_API_CALL vxDirective(vx_reference reference, vx_enum directive)
{
    ...
    if (Reference::isValidReference(reference) == vx_false_e)
    {
        return VX_ERROR_INVALID_REFERENCE;
    }
