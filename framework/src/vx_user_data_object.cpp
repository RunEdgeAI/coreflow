...
vx_size UserDataObject::objSize() const
{
    return size;
}
...
vx_status UserDataObject::copy(vx_size offset, vx_size size, void *user_ptr, vx_enum usage,
                               vx_enum user_mem_type)
{
    ...
    if (((vx_enum)VX_READ_ONLY == usage) &&
        ((uint64_t)(uintptr_t)nullptr == (uintptr_t)memory.ptrs[0]))
    {
    ...
    if ((size < 1U) || ((offset + size) > this->size))
    {
    ...
    {
        if (allocateUserDataObject() == vx_false_e)
        {
    ...
    if (status == (vx_status)VX_SUCCESS)
    {
        ...
        start_ptr = (vx_uint8 *)&memory.ptrs[0][offset];
...
vx_status UserDataObject::map(vx_size offset, vx_size size, vx_map_id *map_id, void **ptr,
...
vx_status UserDataObject::unmap(vx_map_id map_id)
{
    vx_status status = (vx_status)VX_SUCCESS;
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryUserDataObject(vx_user_data_object user_data_object,
    ...
    else
    {
        switch (attribute)
        {
            case (vx_enum)VX_USER_DATA_OBJECT_NAME:
                ...
                {
                    strncpy(reinterpret_cast<char *>(ptr), user_data_object->typeName(),
            ...
            case (vx_enum)VX_USER_DATA_OBJECT_SIZE:
                ...
                {
                    *(vx_size *)ptr = user_data_object->objSize();
                }
                else
                {
        ...
        vx_size offset, vx_size size, void *user_ptr, vx_enum usage, vx_enum user_mem_type)
{
    ...
    {
        status = user_data_object->copy(offset, size, user_ptr, usage, user_mem_type);
    }
    ...
    vx_uint32 flags)
{
    ...
    if ((vx_status)VX_SUCCESS == status)
    {
        status = user_data_object->map(offset, size, map_id, ptr, usage, mem_type, flags);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxUnmapUserDataObject(vx_user_data_object user_data_object, vx_map_id map_id)
{
    ...
    if(status == (vx_status)VX_SUCCESS)
    {
        status = user_data_object->unmap(map_id);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseUserDataObject(vx_user_data_object *user_data_object)
{
    ...
    if (nullptr != user_data_object)
    {
        ...
        {
            status = Reference::releaseReference((vx_reference *)user_data_object,
