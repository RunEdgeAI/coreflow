               ...
               vx_image result_image, void *pBuffer)
{
    ...
    for( x = 0; x < result_width; x++ )
    {
        ...
        if( x > 0 )
        {
            ...
            {
                vx_uint8* srcp = (vx_uint8*)vxFormatImagePatchAddress1d(src, (vx_int32)source_x, &addr);
                dst[0] = srcp[0];
            }
        }
        ...
        for( y = 0; y < result_height; y++ )
        {
            ...
            vx_int16 *result_srcp = (vx_int16*)vxFormatImagePatchAddress1d(result_src, x+y*result_width, &result_addr);
               ...
               vx_image result_image, void *pBuffer)
{
    ...
    for( x = 0; x < result_width; x++ )
    {
        ...
        {
            vx_float64 res = winCoeff * ((vx_float64)resNum[y]);
            vx_int16 *result_srcp = (vx_int16*)vxFormatImagePatchAddress2d(result_src, x, y, &result_addr);
            *result_srcp = (vx_int16)res;
        }
    }
               ...
               vx_image result_image, void *pBuffer)
{
    ...
    for( x = 0; x < result_width; x++ )
    {
        ...
        if( x > 0 )
        {
            ...
            for( y = 0; y < (vx_int32)src_height; y++, source_x+=src_width, dst += template_width )
            {
                vx_uint8* srcp = (vx_uint8*)vxFormatImagePatchAddress1d(src, (vx_uint32)source_x, &addr);
                dst[0] = srcp[0];
            }
        }
