    ...
    if ((src_format == VX_DF_IMAGE_RGB) || (src_format == VX_DF_IMAGE_RGBX))
    {
        if (dst_format == VX_DF_IMAGE_RGB || dst_format == VX_DF_IMAGE_RGBX)
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                ...
                {
                    vx_uint8 *src_rgb = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]);
                    vx_uint8 *dst_rgb = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
                    dst_rgb[0] = src_rgb[0];
                    dst_rgb[1] = src_rgb[1];
                    dst_rgb[2] = src_rgb[2];
        ...
        else if (dst_format == VX_DF_IMAGE_NV12)
        {
            ...
            for (y = 0; y < dst_addr[0].dim_y; y+=2)
            {
                ...
                {
                    vx_uint8 *rgb[4] = {(vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                                        (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x+1, y, &src_addr[0]),
                                        (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y+1, &src_addr[0]),
                                        (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x+1, y+1, &src_addr[0])};
                    vx_uint8 *luma[4] = {(vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x+1, y, &dst_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y+1, &dst_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x+1, y+1, &dst_addr[0])};
                    vx_uint8 *cbcr = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
        ...
        else if (dst_format == VX_DF_IMAGE_YUV4)
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                for (x = 0; x < dst_addr[0].dim_x; x++)
                {
                    vx_uint8 *rgb  = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]);
                    vx_uint8 *luma = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
                    vx_uint8 *cb   = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
                    vx_uint8 *cr   = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[2], x, y, &dst_addr[2]);
        ...
        else if (dst_format == VX_DF_IMAGE_IYUV)
        {
            ...
            for (y = 0; y < dst_addr[0].dim_y; y+=2)
            {
                for (x = 0; x < dst_addr[0].dim_x; x+=2)
                {
                    ...
                    vx_uint8 *cbp = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
                    vx_uint8 *crp = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[2], x, y, &dst_addr[2]);
        ...
        if ((dst_format == VX_DF_IMAGE_RGB) || (dst_format == VX_DF_IMAGE_RGBX))
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                for (x = 0; x < dst_addr[0].dim_x; x++)
                {
                    ...
                    vx_uint8 *crcb = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]);
                    vx_uint8 *rgb = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
                    if (dst_format == VX_DF_IMAGE_RGBX)
                        rgb[3] = 255;
        ...
        else if (dst_format == VX_DF_IMAGE_YUV4)
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                for (x = 0; x < dst_addr[0].dim_x; x++)
                {
                    vx_uint8 *luma = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]);
                    vx_uint8 *crcb = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]);
                    vx_uint8 *yout = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
                    vx_uint8 *cb   = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
                    vx_uint8 *cr   = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[2], x, y, &dst_addr[2]);
    ...
    else if (src_format == VX_DF_IMAGE_YUYV)
    {
        if (dst_format == VX_DF_IMAGE_RGB)
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                for (x = 0; x < dst_addr[0].dim_x; x+=2)
                {
                    vx_uint8 *yuyv = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]);
                    vx_uint8 *rgb  = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
        ...
        else if (dst_format == VX_DF_IMAGE_NV12)
        {
            for (y = 0; y < dst_addr[0].dim_y; y+=2)
            {
                for (x = 0; x < dst_addr[0].dim_x; x+=2)
                {
                    vx_uint8 *yuyv[2] = {(vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                                          (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y+1, &src_addr[0])};
                    vx_uint8 *luma[4] = {(vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]),
                                          (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x+1, y, &dst_addr[0]),
                                          (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y+1, &dst_addr[0]),
    ...
    else if (src_format == VX_DF_IMAGE_UYVY)
    {
        if (dst_format == VX_DF_IMAGE_RGB)
        {
            for (y = 0; y < dst_addr[0].dim_y; y++)
            {
                for (x = 0; x < dst_addr[0].dim_x; x+=2)
                {
                    vx_uint8 *uyvy = (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]);
                    vx_uint8 *rgb  = (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
        ...
        else if (dst_format == VX_DF_IMAGE_NV12)
        {
            for (y = 0; y < dst_addr[0].dim_y; y+=2)
            {
                for (x = 0; x < dst_addr[0].dim_x; x+=2)
                {
                    vx_uint8 *uyvy[2] = {(vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y+1, &src_addr[0])};
                    vx_uint8 *luma[4] = {(vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x+1, y, &dst_addr[0]),
                                         (vx_uint8*)vxFormatImagePatchAddress2d(dst_base[0], x, y+1, &dst_addr[0]),
