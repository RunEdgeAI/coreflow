    ...
    if ((format == VX_DF_IMAGE_RGB) || (format == VX_DF_IMAGE_RGBX))
    {
        ...
        for (y = 0; y < dst_addr.dim_y; y+=dst_addr.step_y)
        {
            for (x = 0; x < dst_addr.dim_x; x+=dst_addr.step_x)
            {
                ...
                if (format == VX_DF_IMAGE_RGBX)
                {
                    planes[3] = (uint8_t*)vxFormatImagePatchAddress2d(base_src_ptrs[3], x, y, &src_addrs[3]);
                    dst[3] = planes[3][0];
                }
            }
        }
        ...
        for (p = 0; p < 3; p++)
        {
            ...
            for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y)
            {
                for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x)
                {
                    ...
                    uint8_t *src = (uint8_t*)vxFormatImagePatchAddress2d(base_src_ptr, x1, y1, &src_addr);
                    uint8_t *dst = (uint8_t*)vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
                    *dst = *src;
                }
            }
    ...
    else if ((format == VX_DF_IMAGE_NV12) || (format == VX_DF_IMAGE_NV21))
    {
        ...
        //plane 0
        {
            ...
            for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y)
            {
                for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x)
                {
                    uint8_t *src = (uint8_t*)vxFormatImagePatchAddress2d(base_src_ptr, x, y, &src_addr);
                    uint8_t *dst = (uint8_t*)vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
                    *dst = *src;
                }
            }
        ...
        // plane 1
        {
            ...
            for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y)
            {
                for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x)
                {
                    ...
                    uint8_t *src0 = (uint8_t*)vxFormatImagePatchAddress2d(base_src0_ptr, x1, y1, &src0_addr);
                    uint8_t *src1 = (uint8_t*)vxFormatImagePatchAddress2d(base_src1_ptr, x1, y1, &src1_addr);
                    uint8_t *dst = (uint8_t*)vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
                    dst[1-vidx] = *src0;
                    dst[vidx]   = *src1;
                }
    ...
    else if ((format == VX_DF_IMAGE_YUYV) || (format == VX_DF_IMAGE_UYVY))
    {
        ...
        for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y)
        {
            for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x * 2)
            {
                ...
                uint8_t *srcy0 = (uint8_t*)vxFormatImagePatchAddress2d(base_src0_ptr, x1, y1, &src0_addr);
                uint8_t *srcy1 = (uint8_t*)vxFormatImagePatchAddress2d(base_src0_ptr, x1 + src0_addr.step_x, y1, &src0_addr);
                uint8_t *srcu = (uint8_t*)vxFormatImagePatchAddress2d(base_src1_ptr, x2, y2, &src1_addr);
                uint8_t *srcv = (uint8_t*)vxFormatImagePatchAddress2d(base_src2_ptr, x2, y2, &src2_addr);
                uint8_t *dst0 = (uint8_t*)vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
                uint8_t *dst1 = (uint8_t*)vxFormatImagePatchAddress2d(base_dst_ptr, x + dst_addr.step_x, y, &dst_addr);
                dst0[yidx] = *srcy0;
                dst1[yidx] = *srcy1;
                dst0[1-yidx] = *srcu;
                dst1[1-yidx] = *srcv;
    ...
    if (status == VX_SUCCESS)
    {
        ...
        if (status == VX_SUCCESS)
        {
            for (y = 0; y < dst_addr.dim_y; y++)
            {
                ...
                {
                    vx_uint8 *srcp = (uint8_t*)vxFormatImagePatchAddress2d(src_base,
                        x * VX_SCALE_UNITY / src_addr.scale_x * x_subsampling,
                        y * VX_SCALE_UNITY / src_addr.scale_y,
                        &src_addr);
                    vx_uint8 *dstp = (uint8_t*)vxFormatImagePatchAddress2d(dst_base, x, y, &dst_addr);
