...
vx_status vxAbsDiff(vx_image in1, vx_image in2, vx_image output)
{
    ...
    for (y = 0; y < height; y++)
    {
        for (x = 0; x < width; x++)
        {
            ...
            {
                vx_uint8 *src[2] = {
                    (vx_uint8*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                    (vx_uint8*)vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]),
                };
            ...
            {
                vx_int16 *src[2] = {
                    (vx_int16*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                    (vx_int16*)vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]),
                };
            ...
            {
                vx_uint16 *src[2] = {
                    (vx_uint16*)vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
                    (vx_uint16*)vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]),
                };
                vx_uint16 *dst = (vx_uint16*)vxFormatImagePatchAddress2d(dst_base, x, y, &dst_addr);
                if (*src[0] > *src[1])
                    *dst = *src[0] - *src[1];
