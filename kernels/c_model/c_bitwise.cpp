...
vx_status vxNot(vx_image input, vx_image output)
{
    ...
    for (y = 0; y < height; y++)
    {
        for (x = 0; x < width; x++)
        {
            ...
            vx_uint8 *src = (vx_uint8*)(vx_uint8*)vxFormatImagePatchAddress2d(src_base, xShftd, y, &src_addr);
            vx_uint8 *dst = (vx_uint8*)(vx_uint8*)vxFormatImagePatchAddress2d(dst_base, xShftd, y, &dst_addr);
